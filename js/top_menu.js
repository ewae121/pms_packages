...
function topMenuSetConnection(connectionState){
   ...
   if (connectionState == 'Disconnected'){
      connectionState = '<i class="fa fa-refresh" id="server-connection-refresh" style="cursor:pointer" aria-hidden="true"></i><span id="server-connection-status" style="margin-left:15px">Disconnected</span>';
      $(topMenuRightConnectionId).html(connectionState);
      $('#server-connection-refresh').click(function(){wsConnect();});
   }else if (connectionState == 'Connecting'){
      connectionState = '<i class="fa fa-refresh fa-spin" id="server-connection-refresh" style="cursor:pointer" aria-hidden="true"></i><span id="server-connection-status" style="margin-left:15px">Connecting ...</span>';
      $(topMenuRightConnectionId).html(connectionState);
   }else{
      $(topMenuRightConnectionId).html(connectionState);
   }
