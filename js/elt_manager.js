...
function _eltManagerInit(proc){
   ...
   elt_step_menu_select_action();
   elt_step_button_action();
}

function elt_step_button_action(){
   $('.elt-button-check').click(function(e){
      e.stopImmediatePropagation();
      e.preventDefault();

      var eltId = $(this).parent().parent().parent().parent().attr('id');
      console.log(eltId);
      sendMessageToPms('execute', {'id':eltId, 'operation': 'check'});
   });

   $('.elt-button-exec').click(function(e){
      e.stopImmediatePropagation();
      e.preventDefault();
      ...
      console.log(eltId);
      if ($(this).text() != 'Stop')
        sendMessageToPms('execute', {'id':eltId, 'operation': 'exec'});
      else
        sendMessageToPms('stop', {'id':eltId});
   });
}
...
function setEltStatus(eltId, eltStatus, force = false){
   ...
   if (eltStatus == ELT_STATUS_RUNNING)
      $("."+eltId+'.elt-button-check').addClass('w3-disabled');
   else
      $("."+eltId+'.elt-button-check').removeClass('w3-disabled');
}

function setEltTitleIcon(eltId, eltStatus){
   var eltTitleTextIconId = '#'+eltId+'-title-text-icon';

   var icon = getStatusClasses(eltId, eltStatus);
   $(eltTitleTextIconId).append(icon);
}
...
function getStatusClasses(eltId, eltStatus){
   var elt = _elements[eltId];
   var statusImg = '<img src=share/png/{0} height="24" width="24" alt="status">';
   if (eltStatus == ELT_STATUS_RUNNING) {
      statusImg = '<img src=share/png/{0} class="spin" height="24" width="24" alt="status">';
      return statusImg.format("running_24x24.png");
   } else if(eltStatus == ELT_STATUS_SUCCEED)
      return statusImg.format("succeed_24x24.png");
   else if(eltStatus == ELT_STATUS_FAILED)
      return statusImg.format("failed_24x24.png");
   else if (eltStatus == ELT_STATUS_NOT_EXECUTED)
      if (elt['type'] == "procedure")
        return statusImg.format("not_executed_24x24.png");
      else
        return statusImg.format("circle_24x24.png");
...
function removeEltColor(eltId){
   $("."+eltId).removeClass('w3-blue w3-green w3-red w3-gray w3-pale-blue w3-pale-green w3-pale-red w3-light-grey');
   $("."+eltId).removeClass('w3-border-blue w3-border-green w3-border-red w3-border-gray w3-border-pale-blue w3-border-pale-green w3-border-pale-red w3-border-light-grey');
}

function removeEltTitleIcon(eltId){
   var eltTitleTextIconId = '#'+eltId+'-title-text-icon';
   $(eltTitleTextIconId).empty();
}
