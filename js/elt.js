...
function _eltGenDescription(elt){
   ...
   var eltDescStyle = "";
   addDivToDiv(eltId, eltDescId, eltDescClasses, eltDescStyle, "<p>"+elt['desc']+"</p>");
}
...
function getDelay(elt) {
   var delay = '';
   if ('operations' in elt)
      if ('status' in elt['operations'])
         if ('delay' in elt['operations']['status'])
            delay = elt['operations']['status']['delay'];
            if (delay == null)
               delay = '';
   return delay;
}
...
function getDelayString(eltId, delay, isPaused) {
   ...
   if (isPaused == false)
      return '<i id="'+eltTitleDelayIconId+'" class="far fa-play-circle w3-margin-right"></i>' + delay;
   else
      return '<i id="'+eltTitleDelayIconId+'" class="far fa-pause-circle w3-margin-right"></i>' + delay;
}
...
function _eltGenContent(elt){
   ...
   if (elt['type'] == 'procedure'
      ...
      var eltCmdId = eltContentId+'-step';
      var eltCmdClasses = eltId + ' w3-code w3-border-blue';
      addDivToDiv(eltContentId, eltCmdId, eltCmdClasses, "", elt['operations'][operation]['cmd']);
